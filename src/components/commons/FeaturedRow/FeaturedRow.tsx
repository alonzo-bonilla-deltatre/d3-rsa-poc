...
function renderSection(props: FeaturedRowProps) {
  ...
  return (
    <SectionContainer className={`d3-section ${getStringProperty(sectionClassName)}`}>
      <div
        className={`flex flex-col lg:grid lg:grid-cols-4 lg:gap-6 lg:max-w-full ${
          !hasFullWidthContent ? 'container' : ''
        }`}
      >
        {showHeader && (
          <div
            className={`d3-section__header ${getStringProperty(
              headerClassName
            ...
            <div className={'flex md:justify-between mb-6'}>
              <div className="w-full">
                <HeaderTitle
                  ...
                  className={'d3-ty-heading-1 mb-6'}
                ></HeaderTitle>
                <div>
                  {featuredDescription && (
                    <div className={'d3-ty-body-small mb-6 text-grey-500 dark:text-grey-100'}>
                      {featuredDescription}
                    </div>
                  )}

                  {featuredSponsor && (
                    <div className={'mb-6 d3-ty-tag-small uppercase flex items-center text-grey-100 dark:text-white'}>
        ...
        {children && (
          <div
            className={`d3-section__content ${getStringProperty(
              contentClassName
