...
export const getGridClasses = (index: number) => {
  ...
  if (count === 0) {
    ...
    return `md:row-span-1 md:col-span-1 md:col-start-1 md:row-start-${row}`;
  } else if (count % 4 === 0) {
    return `md:row-span-2 md:col-span-2 md:row-start-${row} md:col-start-2`;
  } else if (count % 5 === 0) {
    return `md:row-span-1 md:col-span-1 md:col-start-1 md:row-start-${row + 1}`;
  } else if (count % 6 === 0) {
    return `md:row-span-2 md:col-span-2`;
  } else {
...
const MixedSquareGrid = ({ items, cardsType }: MixedSquareGridProps) => {
  ...
  const gridClassName = `grid grid-cols-2 grid-rows-2 gap-2 md:gap-10 md:grid-cols-3`;
