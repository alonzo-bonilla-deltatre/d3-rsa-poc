    ...
    <Link href={entity.url} className={'w-full h-full'}>
      <div className="relative rounded-lg">
        <div className={'absolute rounded-lg bg-event-mask bg-bottom w-full h-[234px] px-6 pt-24 z-10'}>
          <div
            className="flex flex-col justify-items-center center absolute top-0 left-2 lg:left-4 bg-event-date w-[64px] h-[82px] z-10">
            <Typography variant={'caption-l'} className="text-center">{dateFromDay}</Typography>
            <Typography variant={'caption-s'} className="uppercase text-center -mt-1">{dateFromMonth}</Typography>
          </div>
          <div
            className={'absolute gap-2 p-2 lg:px-4 lg:py-2 rounded-full flex items-center z-10 top-2 lg:top-3 right-2 lg:right-3 py-4 text-white bg-grey-500'}>{renderSvgIcon('Event', {
            className: 'w-5 h-5',
            width: 20,
            height: 20,
          })}</div>
