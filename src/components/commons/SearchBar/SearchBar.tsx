...
const SearchBar = ({
  ...
  return (
    <div
      className={twMerge('flex h-full items-center justify-center', widthCssClass, additionalClasses)}
    >
      <div className="w-3/4 px-1 pt-2 flex flex-col justify-center h-full">
        <input
          className={twMerge('bg-transparent border-b-link w-full h-12 leading-5 border-b-2 outline-0 text-2xl placeholder-current', inputAdditionalClasses)}
          autoFocus={true}
          title={inputTitle}
        ...
        />
        {showResultsCount && (
          <div className="flex justify-end py-2">{results} Results found</div>
        )}
      </div>
    </div>
  );
