...
type FeaturedPlayerListCarouselProps = EditorialModuleProps & CarouselProps;
...
const FeaturedPlayerListCarousel = async ({ data }: { data: ComponentProps }) => {
  ...
  const hasPagination = getOppositeBooleanProperty(true);
  const hasNavigation = getOppositeBooleanProperty(hideNavigation);

  const cardType = CardType.Player;
  const cardLayout = CardLayout.PortraitFull;
...
export default FeaturedPlayerListCarousel;
