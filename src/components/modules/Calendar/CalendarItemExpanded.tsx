...
const CalendarItemExpanded = ({ ...data }: ModuleProps) => {
  ...
  const ctaClasses =
    "inline-block text-white bg-[#252525] font-bold uppercase px-8 py-2 rounded-full";

  return (
    <div
      className={`${data.className} flex flex-col justify-center text-center`}
    >
      ...
      <span className="pt-6 text-lg">
        {`${getEventMonthString(fields.dateFrom)} ${getEventDayString(
          fields.dateFrom
        )} - ${getEventDayString(fields.dateTo)} ${eventDate.getFullYear()}`}
      </span>
      ...
      {(fields.enableTickets || fields.enableDetails) && (
        <div className="mt-8">
          <ul className="list-none flex space-x-5 justify-center">
            {fields.enableTickets && fields.buyTicket.url && (
              <li>{renderLink(fields.buyTicket, ctaClasses)}</li>
            )}
            {fields.enableDetails && fields.eventDetails.url && (
              <li>{renderLink(fields.eventDetails, ctaClasses)}</li>
            )}
          </ul>
        </div>
      )}
