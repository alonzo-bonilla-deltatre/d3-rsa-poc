...
const Promo = async ({ ...data }: ComponentProps) => {
  ...
  if (!Object.hasOwn(properties, 'slug') || !properties.slug?.length) {
    const invalidSlugErrorMessage = 'Cannot render Promo module with empty slug';
  ...
  if (promoEntity == null) {
    logger.log(`Cannot find Promo entity with slug ${properties?.slug} `, LoggerLevel.warning);
    notFound();
  }
